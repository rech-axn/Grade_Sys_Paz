<x-layouts.app title="Student Grade Report">
    <div class="mb-6 no-print flex items-center justify-between">
        <a href="{{ route('teacher.students.index') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition flex items-center gap-1.5">
    ...
    <div class="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200/80 shadow-xs max-w-4xl mx-auto print-container">
        ...
        <div class="text-center pb-6 mb-8 border-b border-slate-200">
            ...
            <p class="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Office of the University Registrar & Academic Affairs</p>
            <div class="inline-block mt-3 px-3 py-1 bg-slate-100 text-slate-700 text-xs font-bold rounded-md">
                Official Certificate of Student Grades
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            ...
            <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100">
                <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Program & Section</div>
                <div class="text-base font-bold text-slate-900 mt-0.5">{{ $student->course_section }} ({{ $student->year_level }})</div>

                <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mt-3">Academic Term</div>
                <div class="text-sm font-semibold text-slate-700">A.Y. 2025-2026 | 1st Semester</div>
            </div>
        </div>
        ...
        <div class="border border-slate-200 rounded-2xl overflow-hidden mb-8">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-6 text-center text-slate-400">No grades registered for this student.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-wrap items-center justify-between gap-4 mb-12">
            <div>
                <div class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Enrolled / Passed Units</div>
                <div class="text-sm font-bold text-slate-800 mt-0.5">{{ $student->passed_units }} of {{ $student->total_units }} Credits Passed</div>
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 gap-8 pt-8 border-t border-slate-200">
            <div class="text-center">
                <div class="border-b border-slate-400 pb-1 mb-1.5 font-bold text-slate-900">{{ auth()->user()->name }}</div>
                <div class="text-[11px] text-slate-400 uppercase tracking-wider font-semibold">Faculty Adviser / Instructor</div>
            </div>
            <div class="text-center">
                <div class="border-b border-slate-400 pb-1 mb-1.5 font-bold text-slate-900">Dr. Emmanuel Reyes, Ph.D.</div>
                <div class="text-[11px] text-slate-400 uppercase tracking-wider font-semibold">University Registrar</div>
            </div>
        </div>
    </div>
</x-layouts.app>
